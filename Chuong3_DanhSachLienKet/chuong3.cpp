...
string ChuyenSoThanhChuoi(int n) {
    stringstream ss;
    ss << n;
    return ss.str();
}
...
//9- Tim phan tu be nhat
int TimMin(const List& L) {
    if (DanhSachRong(L)) return -1;
    int mi = L.head->data;
    Node* cur = L.head->next;
    while (cur != nullptr) {
        if (cur->data < mi) mi = cur->data;
        cur = cur->next;
    }
    return mi;
}
